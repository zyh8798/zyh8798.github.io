...
浏览器收到 HTML 后，会先解析 HTML 生成 DOM Tree，同时解析 CSS 生成 CSSOM Tree。
...
最终将两者合并生成 Render Tree（渲染树）。

需要注意的是，JavaScript 的执行与渲染都运行在 Renderer Process 中的 Main Thread 上，两者会相互影响。当解析 HTML 过程中遇到同步 script 标签时，会暂停 HTML 解析，先执行 JavaScript，再继续解析。

---

### Layout（布局）

通过 Render Tree 计算每个元素的最终布局信息。

这里不仅仅是简单的数学计算，更准确地说是在给整个页面进行空间布局。
...
最终得到每个元素在页面中的准确位置和尺寸。

因此它叫 Layout（布局），而不是单纯的计算阶段。

---

### Paint（绘制）

根据 Layout 阶段计算出的布局信息生成绘制指令（Paint Records）。
...
此时浏览器只是记录"应该怎么画"，还没有真正生成像素。

---

### Raster（栅格化）

Paint 阶段产生的绘制指令无法直接显示到屏幕。

GPU 和显示器最终只认识像素（Bitmap，位图）。

因此浏览器会将 Paint Records 转换为真实像素数据。
...
Chrome 会使用 Renderer Process 中的 Raster Thread（栅格线程）来完成这项工作。

---

### Composite（合成）

浏览器会将页面拆分为多个 Layer（图层）。
...
Composite 阶段会将这些已经栅格化完成的 Layer 按照层级关系、位置、透明度等信息进行合成，生成最终页面。

Chrome 会使用 Compositor Thread（合成线程）完成这项工作，并最终交由 GPU 显示到屏幕。
...
因此 transform 动画通常比 left / top 动画性能更好。
